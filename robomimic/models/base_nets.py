...
###### RESNET 18 BASED DECODER MODULES ####
...
class ResNet18Dec(ConvBase):
    ...
    def output_shape(self, input_shape):
        ...
        Returns:
            out_shape ([int]): list of integers corresponding to output shape
        """
        return [self.nc, self.hw, self.hw]
    ...
    def forward(self, z):
        ...
        # print(x.shape)
        x = F.interpolate(x, scale_factor=5) #THIS IS NOT ADAPTATIVE USE AT YOUR OWN RISK
        # print(x.shape)
        x = self.layer4(x)
        # print(x.shape)
        x = self.layer3(x)
